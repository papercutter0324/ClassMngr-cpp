if(NOT DEFINED CLASSMNGR_PROJECT_SOURCE_DIR
   OR NOT DEFINED CLASSMNGR_RESOURCE_MANIFEST)
    message(FATAL_ERROR
        "The resource manifest verifier requires project and manifest paths."
    )
endif()
...
if(NOT EXISTS "${CLASSMNGR_RESOURCE_MANIFEST}")
    message(FATAL_ERROR
        "Resource manifest does not exist: "
        "${CLASSMNGR_RESOURCE_MANIFEST}"
    )
endif()

file(READ "${CLASSMNGR_RESOURCE_MANIFEST}" classmngr_manifest_json)
...
if(NOT classmngr_manifest_count EQUAL classmngr_expected_count)
    message(FATAL_ERROR
        "Resource manifest has ${classmngr_manifest_count} entries; "
        "catalog has ${classmngr_expected_count}."
    )
endif()

if(classmngr_expected_count GREATER 0)
    ...
    foreach(classmngr_index RANGE 0 ${classmngr_last_index})
        ...
        if(NOT classmngr_manifest_size EQUAL classmngr_expected_size)
            message(FATAL_ERROR
                "Resource size mismatch for '${classmngr_expected_source}'."
            )
        endif()
        ...
        if(NOT classmngr_manifest_sha256 STREQUAL classmngr_expected_sha256)
            message(FATAL_ERROR
                "Resource hash mismatch for '${classmngr_expected_source}'."
            )
        endif()
    endforeach()
endif()
...
message(STATUS
    "Verified ${classmngr_manifest_count} resource entries against the catalog."
)
