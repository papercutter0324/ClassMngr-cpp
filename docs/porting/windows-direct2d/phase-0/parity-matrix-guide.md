...
Each feature has independent values for data read/write, input/accessibility,
visual state, error behavior, print/export, and performance on x64 and ARM64.
`not-assessed` means evidence has not yet been captured; `not-applicable` must
be justified by the feature contract rather than used as a shortcut.
...
For each platform and architecture, record the artifact reference and outcome
in the relevant cell. Use these values consistently:
...
The status cannot be promoted from `not-started` until every applicable cell
has a reviewed x64 and ARM64 result. A feature can be `pass` only after the
same copied database fixture or named capture state was exercised where the
row requires data, text input, output, or error behavior.
...
comparison rule. The eventual cutover gate requires every applicable field to
have reviewed x64 and ARM64 evidence; source code citations alone never satisfy
that gate.
