...
| Linux x64 Debug (GitHub-hosted) | `89297e1` | 20.572 s | 718.911 s | 54/54 passed in 21.535 s | 204,297,376 bytes | `artifacts/baseline/linux-gcc-debug.json` |
...
The Linux hosted run used Ubuntu's `Linux-6.17.0-1022-azure` runner, CMake
3.31.6, Qt 6.11.1, Python 3.13.15, and two build jobs at commit `89297e1`.
Its working tree was dirty at both ends of the run, but its recorded source
fingerprint remained unchanged. The complete 54-test suite passed; the Ninja
compilation database recorded 274 production entries for 274 unique sources,
