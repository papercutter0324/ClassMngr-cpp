...
| Schedule workbook `.xlsx` | Regular/intensive schedule import workflow | Read workbook, select sheet/user/mode, review conflicts, apply or cancel | `schedule_import_tests`, `schedule_import_dialog_tests`, permanent `tests/fixtures/imports/schedule_review.xlsx` and `tests/fixtures/imports/schedule_large_conflict.xlsx`, plus offscreen staged-review/conflict captures | Golden large schedule output and apply/rollback comparison |
