...
| Substitute preparation and output | `SubPrepPage`, package/print services | Substitute settings, class information, materials, grading, package generation, roster/sub-prep PDFs | `sub_prep_page_tests`, `sub_prep_class_information_model_tests`, `sub_prep_package_service_tests`, `sub_prep_print_pdf_tests`; heavy 96-class output probe and retained PDF/package reference; packaged large-route trace with feature-specific counts | Inventory complete; current large route measured but exceeds the v2 memory boundary; visual/parity and v2 ownership gate pending |
