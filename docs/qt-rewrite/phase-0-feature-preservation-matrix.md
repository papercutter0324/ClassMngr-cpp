...
| Classes | `ClassesPage` | Class selection, day/grade filters, Details/Roster/Analytics/Evaluations/Co-Teacher/Notes, class CRUD, class transfer | `classes_page_tests`, `class_transfer_tests`, `class_analytics_ranking_model_tests`, `evaluation_default_selection_tests`, `roster_model_tests`, `speaking_evaluation_service_tests`; representative startup fixture, `tests/fixtures/transfers/conflict_source.json`, and packaged 96-class entry/selected/re-entry English/Korean light/dark visual references | Inventory complete; populated and conflict fixtures added; populated visual states retained; empty/dialog/error coverage pending |
| Schedule and imports | `SchedulePage`, `ScheduleWidget`, import dialogs | Regular/intensive schedule, editor, conflict review, teacher/schedule import, testing-class entry, PDF/print output | `schedule_widget_tests`, `schedule_builder_tests`, `schedule_import_tests`, `schedule_import_dialog_tests`, `schedule_print_model_tests`, `schedule_print_pdf_tests`, `class_transfer_tests`; representative/large workspace fixtures, class-transfer conflict fixture, `tests/fixtures/imports/schedule_review.xlsx`, `tests/fixtures/imports/schedule_large_conflict.xlsx`, and packaged large-route loading/review/source/conflict-warning captures | Inventory complete; regular/intensive, class-transfer conflict, schedule-workbook staged-review/conflict, asynchronous loading, and large-route conflict-warning references retained; generated-output and remaining large-feature evidence pending |
| Calendar | `CalendarPage`, embedded QML `EventCalendar` | Month navigation, event CRUD/import, upcoming events, theme/language behavior, deferred QML construction | `academic_calendar_tests`, `calendar_event_repository_tests`, `calendar_event_cache_tests`, `calendar_import_tests`; packaged large Calendar Import loading, Preferences, Calendar-page captures, and accepted parser-failure/error evidence at `docs/qt-rewrite/visual-baseline/release/large-calendar-import-error-boundary/calendar-import-error.png` | Inventory complete; success, loading, and parser-failure/error boundaries retained; cross-platform capture pending |
...
| Speaking evaluations | `SpeakingEvalPage` and report dialogs/services | Evaluation tables, undo/redo, notes, name matching, analytics, AI prompt workflows, batch reports, PowerPoint/PDF output | `speaking_evaluation_service_tests`, `speaking_analytics_tests`, `speaking_eval_report_widget_tests`, `speaking_eval_batch_report_service_tests`, `powerpoint_data_access_notice_tests`; packaged heavy PDF/archive output and PowerPoint renderer-selection reference | Inventory complete; generated output references retained; external Office automation remains platform-specific and was not executed in the Windows offscreen baseline |
...
| Substitute preparation and output | `SubPrepPage`, package/print services | Substitute settings, class information, materials, grading, package generation, roster/sub-prep PDFs | `sub_prep_page_tests`, `sub_prep_class_information_model_tests`, `sub_prep_package_service_tests`, `sub_prep_print_pdf_tests`; heavy 96-class output probe and retained PDF/package reference; packaged large-route trace with feature-specific counts; editing/read-only plus selected/changed/empty English/Korean light/dark visual references; configured and validation-error generation-dialog captures | Inventory complete; current large route measured but exceeds the v2 memory boundary; loading parity and v2 ownership gate pending |
