...
| --- | --- | --- | --- | --- |
| Setup and workspace/file flows | `MainWindow`, `FileController`, `InitialSetupWizard` | Create/open/save/save-as/close/export, recent files, initial setup, backup/cancel behavior, no-database state | `database_file_format_tests`, `database_schema_manager_tests`, `initial_setup_wizard_tests`, `data_service_lifecycle_tests` | Inventory complete; fixtures pending |
...
| Rosters | `RosterEditorWidget`, `RosterModel` | Dynamic columns/widths, names and validation, duplicate-name handling, row transfer, score import, print/template output | `roster_model_tests`, `roster_template_print_service_tests`, `sub_prep_print_pdf_tests` | Inventory complete; large roster fixture pending |
