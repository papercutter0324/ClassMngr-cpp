...
**Status (2026-09-05): Deferred; local Windows evidence is green for the
available matrix, while the required cross-platform retained-Qt gate remains
externally blocked.**
...
aggregate remains red. The current local focused validations for the newly
completed R03, R06, R01, and R08 slices pass, but the stored seven-lane report
predates those commits and must be regenerated by the owning CI/device lanes.
Since this is currently an unofficial port, the evidence gate remains deferred
rather than treated as a current release blocker.
...
the Windows matrix and retained-Qt report paths, the host-blocked Linux/macOS
results, the aggregate validator outcome, and the MSVC FileTracker host
...
complete until the CI-owned retained-Qt lanes produce runtime-tested reports
for the current source and the aggregate is non-red.
