...
**Status (2026-09-05): Deferred; the macOS retained-Qt lane is now
runtime-tested PASS, but the required cross-platform aggregate and clean
retained-Qt build are still open.**
...
lane. A fresh local macOS run on commit
`9a6c9c964589df48e6d7071b5def1d289e373011` with exact Qt 6.12.0 universal
runtime evidence now passes the retained database-port fixture in both
directions. The same macOS host's clean full Qt baseline configures
successfully but stops compiling the stale `ClassMngrClassTransferTests`
constructor calls, and the broader retained-Qt sweep has 11 failing targets;
Linux remains host-blocked. The stored seven-lane aggregate therefore remains
open and must be regenerated by the owning CI/device lanes. Since this is
currently an unofficial port, the evidence gate remains deferred rather than
treated as a current release blocker.
...
- [ ] Record retained-Qt adapter results on the required Qt version and keep
  ...
  direction, for every migrated persistence slice. The Windows and macOS
  retained-Qt reports are now exact Qt 6.12.0 runtime-tested; the macOS fixture
  covers all seven persisted slices in both directions. Linux remains
  host-blocked with an explicit missing-prefix reason, and the broader clean
  macOS Qt sweep still needs the constructor and runtime failures repaired.
...
the Windows matrix, the runtime-tested macOS lane, the host-blocked Linux
result, the aggregate validator outcome, and the MSVC FileTracker host
limitation. The plan's Validation section still requires cross-platform
fixture readability and a complete migrated-feature matrix; P2-R07 is not
complete until the CI-owned retained-Qt lanes produce current runtime-tested
reports, the clean retained-Qt suite is green, and the aggregate is non-red.
