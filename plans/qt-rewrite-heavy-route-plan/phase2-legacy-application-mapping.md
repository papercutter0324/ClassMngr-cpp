...
| `scheduleService()` | Menu, schedule UI, testing-class UI, and roster output consume schedule/testing operations; representative calls are [`menu_builder.cpp`](../../src/app/menu_builder.cpp#L195) and [`schedule_widget.cpp`](../../src/features/schedule/ui/schedule_widget.cpp#L307). | Partial F37/F38/F39/F40/F41/F47/F48/F58/F59/F60/F62/F63 boundary: `Application::validateScheduleImportState` validates the adapted plan and snapshots before writes; F59 types its apply-time teacher/class targets, snapshots, and class-to-teacher links as `Domain::TeacherId`/`Domain::ClassId`. The `ScheduleImportRepository` adapter performs action-aware legacy sentinel conversion at the `scheduleService()` edge. F60 types Schedule Import review-decision request and issue targets as optional `Domain::ClassId`; `PlanValidator` and dialog feature edges convert legacy integers and represent nonpositive IDs as absent. F62 characterizes reachable apply-boundary action/sentinel behavior for teacher and class targets, including unchanged persisted snapshots for rejected cases; CreateNew class sentinel conversion is not isolated because F60 PlanValidator normalizes nonpositive IDs and state validation ignores CreateNew targets. [`ScheduleImportMatchingProjection`](../../src/next/application/schedule_import_matching_projection.h) powers [`ScheduleImportRepository::preview`](../../src/data/repositories/schedule_import_repository.cpp), replacing the legacy matcher. F58 types its app-less teacher/class matching keys as `Domain::TeacherId`/`Domain::ClassId` and returns the suggested class as optional; the legacy repository adapter converts numeric IDs back to the existing integer preview. F63 adds a production workbook assertion for the reachable no-suggestion `-1` sentinel (see the verified F63 slice below). F39's [`schedule_import_overlap_projection.h`](../../src/next/application/schedule_import_overlap_projection.h) is shared by review presentation and apply-time validation; F40 supplies `Domain::Weekday`/`Domain::ScheduleTime` values through that projection. F41's [`schedule_import_review_decisions.h`](../../src/next/application/schedule_import_review_decisions.h) validates teacher/class choices for dialog readiness and PlanValidator. F47 adds Course-owned weekly meeting-day policy used by production parse and plan/apply validation; raw parsing and translated diagnostics remain at the feature edge. F48 adds Qt-free [`Domain::ScheduleEntry`](../../src/next/domain/schedule_entry.h), pairing typed `ClassId` and validated `ScheduleTime`; after resolving actual target class IDs, the repository builds entries from final times and passes them to the persistence writer, including retained/skipped rows. The writer preserves original day/time SQL text at the adapter edge and checks it against the typed values; existing write order, transaction, rollback, Skip, and intensive branches remain intact. Qt matching keys are normalized at the edge while raw room text remains for display; translations remain at the UI edge. Required fixtures cover Schedule preview, conflict review/apply, explicit review choices through persisted apply, F47 accepted-pattern persistence/rejected-pattern no-write behavior, F48 typed-entry persistence plus seeded overlap rejection, F59's mismatched-teacher-key Skip rejection, and F60's typed review targets and duplicate claimant/issue detail ordering; broader schedule persistence and other schedule/testing operations remain legacy. | Application validation, matching, overlap projection, typed review-decision targets, and typed Domain time/course/schedule-entry rules plus legacy repository/UI edges; remaining schedule feature and persistence behavior. |
...
Executor focused CTest passed 1/1; no full suite was run. As of F58, a direct
production assertion for the adapter's no-suggestion `-1` sentinel was absent;
F63 closes this specific gap. App-less contract tests already asserted no
suggested class.

## Verified F59 Schedule Import state-validation identity mapping - commit `7769912e1a8ccec02ecc3ace2de11ff98c719327`

At the [`scheduleService()`](../../src/data/repositories/schedule_import_repository.cpp)
boundary, `ScheduleImportRepository` adapts legacy integer targets and snapshots
to Qt-free `Application::validateScheduleImportState` values using
`Domain::TeacherId` and `Domain::ClassId`. The conversion preserves the legacy
action-specific sentinel rules: Reuse/UpdateRoom teacher targets and
UpdateExisting class targets retain their values, including nonpositive
sentinels; Create/Skip teacher targets and CreateNew/Skip class targets are
forwarded only when positive. This conversion stays in the repository adapter,
outside the Qt-free Application contract.

The app-less contract's compile-time checks distinguish the identity types;
behavior tests preserve state-validation/conflict ordering, skip target
exactness and uniqueness, and teacher-key mismatch rejection. Independent fresh
MSVC 19.51/Ninja/Qt 6.12 verification passed exact CTest 2/2:
`ClassMngrNextApplicationScheduleImportStateValidationTests` and
`ClassMngrScheduleImportTests`, including the checked-in workbook apply fixture
and `rejectsSkippedClassWithMismatchedTeacherKey`. No full suite was run. Direct
tests do not exhaustively enumerate all per-action sentinel combinations.

## Verified F60 Schedule Import review-decision identity mapping - commit `730955dd1feb24e5a46dd0bfef9f86b8ff619621`

Schedule Import review-decision request and issue targets are optional
`Domain::ClassId` values. Both `PlanValidator` and the dialog feature edge
convert legacy integer IDs to the typed form, representing nonpositive IDs as
absent. The app-less contract preserves UpdateExisting's required target,
CreateNew's no-target rule, and Skip's optional target, along with duplicate
claimant validation and issue details/order. Dialog checks retain translated
issue text and existing class-label behavior.

Compile-time checks verify typed target fields and separation of `ClassId` from
`TeacherId`. Explicit CreateNew-without-target and Skip-without-target cases,
duplicate update/skip details, and UI label/translation behavior are covered.
...
production Schedule Import test includes
`previewsAndAppliesCheckedInWorkbookAgainstSeededDatabase` using the checked-in
[`schedule_review.xlsx`](../../tests/fixtures/imports/schedule_review.xlsx).
The executor reran the app-less target after the Skip-without-target case was
added (1/1); `git diff --check` passed. No full suite was run. The dialog test
`reviewWarnsForDuplicateClassTargets` directly asserts that the warning includes
the resolved existing class label `E5 Athena`. At F60, the
remaining coverage gap was the F59 action/sentinel matrix.

## Verified F62 Schedule Import apply-boundary sentinel characterization - commit `691e56fbdcc536aaaf577602feeac25fc5b7227f`

[`tests/schedule_import_tests.cpp`](../../tests/schedule_import_tests.cpp)
characterizes reachable Schedule Import repository apply behavior. Teacher
cases cover Reuse/UpdateRoom targets -1/0 with matching snapshot rows present
and absent, and Create/Skip targets -1/0 or a positive foreign ID. Class cases
cover CreateNew sentinels, targetless Skip sentinels, exact/mismatching positive
Skip targets, and stale positive UpdateExisting. Rejected cases compare
persisted snapshots and confirm no writes. Positive CreateNew and nonpositive
UpdateExisting class targets are rejected by F60 PlanValidator before adapter
conversion. CreateNew class sentinel conversion cannot be isolated because the
PlanValidator normalizes nonpositive targets to absence and state validation
ignores that action; these rows prove overall apply behavior only.

Fresh independent x64 Ninja/MSVC 19.51/Qt 6.12 verification built
`ClassMngrNextApplicationScheduleImportStateValidationTests` and
`ClassMngrScheduleImportTests`; the exact CTest filter passed 2/2. The latter
includes the checked-in [`schedule_review.xlsx`](../../tests/fixtures/imports/schedule_review.xlsx)
apply fixture. No full suite was run. Gate 1 and Gate 2 remain Partial; the
Workspace boundary and audited `src/next` dependency isolation remain Satisfied;
Phase 2's exit gate remains Open. F62 adds bounded action/sentinel evidence but
does not exhaust the F59 matrix. Next step: select F63. Sub Prep remains limited
to 2026-2027, the current and following calendar years.

## Verified F63 Schedule Import no-suggestion sentinel assertion - commit `bf4251eca530066ba65b00021f63779d185bd64e`

The production test
`previewsAndAppliesCheckedInWorkbookAgainstSeededDatabase` now asserts its
first candidate M3/Song has empty matching IDs, legacy `suggestedClassId == -1`,
`exactMatch == false`, and confidence None. Seeded classes E4/Hercules and
M2/Atlas make the no-match reachable; the positive-suggestion case remains.
This closes the F58-documented production adapter assertion gap. App-less
projection absence was already covered.

Fresh independent Windows x64 Debug Ninja/MSVC 19.51/Qt 6.12.0 verification
validated 912 handwritten source owners. Executor and Tester each passed exact
`ClassMngrScheduleImportTests` and
`ClassMngrNextApplicationScheduleImportMatchingProjectionTests` CTests 2/2 in
separate fresh trees. No full suite was run. F63 adds Gate 2 adapter evidence;
Gate 1 remains Partial and unchanged. Workspace boundary and audited `src/next`
dependency isolation remain Satisfied; Phase 2 exit gate remains Open. F62's
bounded action/sentinel coverage and CreateNew class conversion limitation
remain. Next entry: select F64; Sub Prep remains capped to 2026-2027, the
current and following calendar years.

## Verified F64 student name-pair Domain value - commit `559b4feaa8fd67c01cd2f4d0f3ddd7dc0f166de5`

Qt-free [`Domain::StudentNamePair`](../../src/next/domain/student_name_pair.h)
stores English and Korean names as separate exact UTF-16 components. Its
factory rejects either empty half; equality and ordering compare the components
without normalization. `RosterEditorWidget::importScores` trims at the Qt edge
before constructing the pair, skips incomplete pairs, and uses the typed pair
key while retaining last-write-wins behavior for duplicate imported score
pairs. Separate components also remove the old delimiter ambiguity for invalid
legacy names containing U+001F.

Domain tests cover empty halves, exact/case/internal-whitespace and UTF-16
semantics, ordering, and delimiter-separated pairs. The real widget-import test
covers trimming when only the roster-side name is padded. It also injects a
duplicate name pair into the second saved speaking-evaluation row (current input
validation rejects new duplicates) and verifies that the later grade persists.
Fresh independent Windows x64 Debug Ninja/MSVC 19.51/Qt 6.12 builds each passed
the exact `ClassMngrNextDomainContractTests` and
`ClassMngrRosterEditorWidgetImportTests` CTests (2/2). No full suite was run.
