...
| `teacherService()` | Navigation, setup/import, roster, and teacher-facing UI; representative call sites are [`navigation_controller.cpp`](../../src/app/controllers/navigation_controller.cpp#L118) and [`initial_setup_wizard.cpp`](../../src/features/setup/ui/initial_setup_wizard.cpp#L154). | Partial F44 Teacher Import decision boundary: Qt-free [`import_review_session.h`](../../src/next/application/import_review_session.h) is shared by dialog readiness/plan creation and repository apply validation. The repository still owns teacher identity checks, source-date handling, and transactional persistence; this does not migrate `TeacherService` or other teacher workflows. | Application decision contract with legacy UI/repository edges; remaining teacher-service behavior. |
| `classService()` | Navigation, classes, roster, schedule, speaking evaluation, and setup use class CRUD/import data; representative calls are [`classes_page.cpp`](../../src/features/classes/ui/classes_page.cpp#L179) and [`navigation_controller.cpp`](../../src/app/controllers/navigation_controller.cpp#L234). | Partial F43 Class Transfer boundary: Qt-free [`class_transfer_projection.h`](../../src/next/application/class_transfer_projection.h) validates shared review decisions for dialog readiness and repository apply. Fixture-backed success and conflict paths are recorded below. Broader class CRUD/import use cases and projections remain legacy/future. | Application decision contract with feature UI/repository edges; remaining class service behavior. |
| `scheduleService()` | Menu, schedule UI, testing-class UI, and roster output consume schedule/testing operations; representative calls are [`menu_builder.cpp`](../../src/app/menu_builder.cpp#L195) and [`schedule_widget.cpp`](../../src/features/schedule/ui/schedule_widget.cpp#L307). | Partial F37/F38/F39/F40/F41 boundary: `Application::validateScheduleImportState` validates the adapted plan and snapshots before writes; [`ScheduleImportMatchingProjection`](../../src/next/application/schedule_import_matching_projection.h) powers [`ScheduleImportRepository::preview`](../../src/data/repositories/schedule_import_repository.cpp), replacing the legacy matcher. F39's [`schedule_import_overlap_projection.h`](../../src/next/application/schedule_import_overlap_projection.h) is shared by review presentation and apply-time validation; F40 supplies `Domain::Weekday`/`Domain::ScheduleTime` values through that projection. F41's [`schedule_import_review_decisions.h`](../../src/next/application/schedule_import_review_decisions.h) validates teacher/class choices for dialog readiness and PlanValidator. Qt matching keys are normalized at the edge while raw room text remains for display; translations remain at the UI edge. Required fixtures cover Schedule preview, conflict review/apply, and explicit review choices through persisted apply; broader schedule persistence and other schedule/testing operations remain legacy. | Application validation, matching, overlap projection, review-decision, and typed Domain time contracts plus legacy repository/UI edges; remaining schedule feature and persistence behavior. |
